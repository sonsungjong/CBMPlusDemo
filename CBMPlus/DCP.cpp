...
void DCP::PieChart(INT a_x, INT a_y, INT a_width, INT a_height, DOUBLE percentage)
{
	// 펜 생성
	Gdiplus::Pen orangePen(Gdiplus::Color(255, 255, 165, 0), 2);			// orange
	Gdiplus::Pen grayPen(Gdiplus::Color(255, 255, 165, 0), 2);				// gray

	// 사각형 생성
	Gdiplus::Rect pieRect(a_x, a_y, a_width, a_height);

	// pie를 그린다
	mp_graphic->DrawPie(&orangePen, pieRect, 0, static_cast<float>(percentage) * 360.0f);
	mp_graphic->DrawPie(&grayPen, pieRect, static_cast<float>(percentage) * 360.0f, 360.0f - static_cast<float>(percentage) * 360.0f);
