...
// 이것은 임시이며, 검색은 무조건 시리얼넘버가 중복되지 않게
// 각 시리얼 넘버에 대한 정보는 그래프와 상세보기를 통해서 나타낸다
void MT_ListCtrl::SetData(EnvironmentModel* ap)
{
	...
	// 대상의 처음위치를 가져옴
	EnvironmentModel* p = ap;

	// 격자 모양 추가 후 컬럼 추가
	SetExtendedStyle(LVS_EX_GRIDLINES | LVS_EX_FULLROWSELECT);							// 격자 추가
	InsertColumn(0, _T("순번"), LVCFMT_CENTER, 50);
	InsertColumn(1, _T("유도탄 SN"), LVCFMT_CENTER, 160);
	InsertColumn(2, _T("일시"), LVCFMT_CENTER, 180);
	InsertColumn(3, _T("온도"), LVCFMT_CENTER, 80);
	InsertColumn(4, _T("습도"), LVCFMT_CENTER, 80);
	InsertColumn(5, _T("진동"), LVCFMT_CENTER, 80);
	InsertColumn(6, _T("위도"), LVCFMT_CENTER, 80);
	InsertColumn(7, _T("경도"), LVCFMT_CENTER, 80);
	InsertColumn(8, _T("리더기 분석값"), LVCFMT_CENTER, 120);
	InsertColumn(9, _T("상세 보기"), LVCFMT_CENTER, 100);
	ModifyStyle(LVS_TYPEMASK, LVS_REPORT);
	

	wchar_t strTemp[32] = { 0, };
	// 항목 삽입
	int listctrl_count = 20000;
	for (int i = 0; i < listctrl_count; i++, p++) {

		swprintf_s(strTemp, 32, L"%d", i + 1);
		InsertItem(i, strTemp);
		SetItem(i, 1, LVIF_TEXT, p->GetSN(), NULL, NULL, NULL, NULL);
		SetItem(i, 2, LVIF_TEXT, p->GetTimeStr().c_str(), NULL, NULL, NULL, NULL);
		swprintf_s(strTemp, 32, L"%.2f ℃", p->GetTemp());
		SetItem(i, 3, LVIF_TEXT,  strTemp, NULL, NULL, NULL, NULL);
		swprintf_s(strTemp, 32, L"%d %%", p->GetHumi());
		SetItem(i, 4, LVIF_TEXT, strTemp, NULL, NULL, NULL, NULL);
		swprintf_s(strTemp, 32, L"%.2f", p->GetVib());
		SetItem(i, 5, LVIF_TEXT, strTemp, NULL, NULL, NULL, NULL);
		swprintf_s(strTemp, 32, L"%.2f", p->GetLat());
		SetItem(i, 6, LVIF_TEXT, strTemp, NULL, NULL, NULL, NULL);
		swprintf_s(strTemp, 32, L"%.2f", p->GetLon());
		SetItem(i, 7, LVIF_TEXT, strTemp, NULL, NULL, NULL, NULL);
		swprintf_s(strTemp, 32, L"%.2f %%", p->GetRel());
		SetItem(i, 8, LVIF_TEXT, strTemp, NULL, NULL, NULL, NULL);
		// SetBtn 9
	}
}
...
void MT_ListCtrl::DrawCtrl(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	if (index >= 0 && index < GetItemCount())
	{
		...
		if (lpDrawItemStruct->itemState & ODS_SELECTED)
		{
			...
			if (m_prev_index != index) {
				m_prev_index = index;

				//GetParent()->SendMessage(SELECT_LISTCTRL_ROW);
				GetParent()->SetTimer(SELECT_LISTCTRL_ROW, 10, NULL);
			}
		}
		else
		{
